...
import { drawHero, drawBullet, drawEnemy } from '../utils/drawManager';
import { changeState } from '../store/actions/game.actions';
import { bulletsUpdate, setScore, damagePlayer } from '../store/actions/player.actions';
...
class GameView extends React.Component {
    ...
    update = () => {
        ...
        this.props.enemies.forEach(b => {
            let x = b.x, y = b.y + this.state.enemySpd;
            ...
            for(let e = 0; e < enemies.length; e++){
                ...
                if(bullets.length >= 1){
                    for(let i = 0; i < bullets.length; i++){
                        if(bullets[i] && enemies[e] && bullets[i].x >= enemies[e].x && 
                ...
                // Check Player
                if(enemies[e] && (this.state.x + 25) >= enemies[e].x && 
    ...
    render(){
        return <div>
                    <div style={{display: 'flex', justifyContent: 'space-between'}}>
                        ...
                        <h1>Lives: {this.props.lives}</h1>
                    </div>
                    <canvas ref={this.gameCanvas} 
                                id="game-canvas"
